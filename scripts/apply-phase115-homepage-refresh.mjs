...
if (source.includes('// EBG_PHASE115_HOMEPAGE_REFRESH')) process.exit(0)

const must = (pattern, replacement, label) => {
  const next = source.replace(pattern, replacement)
  if (next === source) throw new Error(`Phase 1.15 patch failed: ${label}`)
  source = next
}
...
const homePage = `function HomePage({
  ...
  return (
    <main className="page home-v2">
      ...
      <section className="home-section">
        ...
        <div className="home-pulse-grid">
          <article className="home-pulse-card">
            <div>
              <span className="pulse-badge">Casting</span>
              <h3>Step into the story.</h3>
              <p>Open casting opportunities and official EBG submissions live in EBG Forms.</p>
            </div>
            <a href="https://forms.ebgplus.app">View Casting →</a>
          </article>
          <article className="home-pulse-card">
            <div>
              <span className="pulse-badge">Applications</span>
              <h3>Track your application.</h3>
              <p>See your current casting status and follow each application from submission to final decision.</p>
            </div>
            <Link to="/app/my-applications">My Applications →</Link>
          </article>
          <article className="home-pulse-card">
            <div>
              <span className="pulse-badge">{newestEpisode ? 'New Release' : 'Discover'}</span>
              <h3>{newestEpisode ? newestEpisode.title : 'Explore EBG+'}</h3>
              <p>{newestEpisode && newestShow ? \`New from \${newestShow.title}.\` : 'Discover originals, music, specials, and the wider EBG universe.'}</p>
            </div>
      ...
      <section className="home-section">
        <div className="home-section-head">
          <div><h2>Coming Soon</h2><p>A look at what’s next on EBG+.</p></div>
        </div>
          ...
          <div className="home-coming-grid">
            {comingSoonShows.map((show) => (
              <Link key={show.id} className="home-coming-card" to={\`/app/shows/\${show.id}\`} style={{ backgroundImage: \`url(\${show.banner || show.artwork})\` }}>
                <div>
                  <span className="pulse-badge">Coming Soon</span>
                  <h3>{show.title}</h3>
                  <p>{show.genre}</p>
                </div>
              </Link>
...
must(/function HomePage\([\s\S]*?\n\}\n\nfunction ContentCard/, `${homePage}\n\nfunction ContentCard`, 'homepage component refresh')

fs.writeFileSync(path, source)
console.log('Applied Phase 1.15 cinematic homepage refresh')
